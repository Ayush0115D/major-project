  ...
  return (
    <>
      ...
      <div className={`
        ...
        text-white h-screen overflow-y-auto border-r border-slate-700/50 
        backdrop-blur-sm fixed left-0 top-0 z-50 transition-all duration-300
      `}>
        <div className="p-6">
          {/* Logo Section */}
          <div className="flex items-center mb-10 group">
            <div className="p-2 bg-gradient-to-br from-blue-500 to-cyan-500 rounded-xl shadow-lg group-hover:scale-110 transition-transform duration-300 flex-shrink-0">
            ...
            {isOpen && (
              <button
                onClick={toggleSidebar}
                className="ml-auto p-2 hover:bg-slate-700/50 rounded-lg transition-colors duration-200"
              >
          ...
          <div className="mb-6">
            ...
            <div className="space-y-2">
              {navigationItems.map((item) => (
                <div 
                  key={item.id}
                  className={`flex items-center p-3 rounded-xl cursor-pointer transition-all duration-300 group relative ${
                    currentView === item.id 
                ...
                >
                  <div className={`p-2 rounded-lg ${isOpen ? 'mr-3' : 'mx-auto'} ${
                    currentView === item.id 
                  ...
                  {!isOpen && (
                    <div className="absolute left-16 bg-slate-800 text-white px-3 py-2 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity duration-200 pointer-events-none z-50 whitespace-nowrap">
                      {item.label}
                    </div>
                  )}
                </div>
